...
Перейдите в каталог проекта и переключитесь на ветку `Django_lesson3`:
```sh
cd star-burger
...
Чтобы собрать фронтенд, используйте bash-скрипт `deploy_starburger.sh`, который можно найти в корне проекта:
...
Вам потребуется всё тот же bash-скрипт `deploy_starburger.sh`:
