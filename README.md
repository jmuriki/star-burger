...
Определите переменные окружения `SECRET_KEY`, `DB_URL`, `YANDEX_API_KEY`, `ROLLBAR_ACCESS_TOKEN` и `ENVIRONMENT`. Создайте файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
